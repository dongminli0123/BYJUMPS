...
# Based on 500 simulation runs, outputs the value of ARL (ARL), and the standard error (std) under scenarios in Table 2
sig_all = c(0, 0.0025, 0.01, 0.03)
time_horizon_all = c(2,3,5)
h_all = matrix(c(12.571, 12.635, 12.863, 11.232, 11.435, 11.237, 10.554, 10.478, 10.255, 9.019, 9.232, 9.197),4,3, byrow = TRUE)
for (sig_i in 1:4)
{
  for (T_i in 1:3)
  {
    sig = sig_all[sig_i]
    time_horizon = time_horizon_all[T_i]
    h = h_all[sig_i,T_i]
    for (shift in 1:4)
    {
      arl = NULL
      ...
      print(paste0("sigma2 = ", sig, "T = ", time_horizon-1, "shift = ", shift, " ARL = ", average, " std = ", std))
    }
  }
}