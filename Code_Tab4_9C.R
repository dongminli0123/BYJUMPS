...
n=1   #first obs
mu = mu0
OC_loc = c(34,35,36,44,45,46,54,55,56)
...
# Based on 500 simulation runs, outputs the magnitude of mean shift (shift), the value of ARL (ARL), and the standard error (std) of BYJUMPS under scenario 9C
sig_9C = c(0.0025, 0.01, 0.03, 0.03)
time_horizon_9C = c(2,5,3,5)
h_9C = c(11.232, 10.255, 9.232, 9.197)    
for (shift in 1:4)
{
  sig = sig_9C[shift]
  time_horizon = time_horizon_9C[shift]
  h = h_9C[shift]
  arl = NULL
  for (seed in 1:500)
  {
  ...
  print(paste0("9C:shift = ", shift, " ARL = ", average, " std = ", std))
}