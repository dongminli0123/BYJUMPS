simul <- function(seed, jump, p, h, shift)
{
...
delta = matrix(jump, d, 1) #jump size
#p = 0.95
...
# Based on 500 simulation runs, outputs the value of ARL (ARL), and the standard error (std) under scenarios in Table 3
jump_all = c(0.3, 0.5, 0.9)
p_all = c(0.8,0.9,0.95)
h_all = matrix(c(11.189, 10.754, 10.453, 11.154, 10.615, 10.255, 10.763, 10.180, 10.412),3,3, byrow = TRUE)   
for (jump_i in 1:3)
{
  for (p_i in 1:3)
  {
    jump = jump_all[jump_i]
    p = p_all[jump_i]
    h = h_all[jump_i,p_i]
    for (shift in 1:4)
    {
      ...
      for (seed in 1:500)
      {
        l = simul(seed, jump, p, h, shift)
        #print(l)
        arl = c(arl, l)
      }
      average = mean(arl)
      std = sd(arl)/sqrt(length(arl))
      print(paste0("delta = ", jump, "p = ", p, "shift = ", shift, " ARL = ", average, " std = ", std))
    }
  }
}